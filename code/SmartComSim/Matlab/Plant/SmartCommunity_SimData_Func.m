function [] = SmartCommunity_SimData_Func(X_k_Plant_History, U_k_History, E_LoadData, E_Load_Desired, HEMSWeatherData_Output, HEMSPlant_Params, Community_Params, Simulation_Params, result_filefolder_paths)
    %SMARTCOMMUNITY_SIMDATA_FUNC Saves simulation data for Smart Community simulations.
    %
    ...
    % Construct the filename for saving simulation data
    SimulationData_FileName = fullfile(result_filefolder_paths.Results_FolderPath, 'SimulationData', strcat(result_filefolder_paths.SimulationData_FileName,'.mat'));

    % Save the structure to a .mat file
    save(SimulationData_FileName, 'HEMS_Plant_FigurePlotter_Input');

    % Display a confirmation message
    fprintf('Simulation data saved successfully to: %s\n', SimulationData_FileName);
