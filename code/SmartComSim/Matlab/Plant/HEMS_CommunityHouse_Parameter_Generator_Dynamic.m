function [HEMSPlant_Params,HEMSHouse_Params] = HEMS_CommunityHouse_Parameter_Generator_Dynamic(Community_Params,Simulation_Params,Plant_Dynamic_Params)
    % edited by shishir
    ...
    % From Plant Dynamic Params
    hem_D = Plant_Dynamic_Params.hem;
    Lat_D = Plant_Dynamic_Params.Lat;
    Long_D = Plant_Dynamic_Params.Long;
    Ltm_D = Plant_Dynamic_Params.Ltm;

    AC_COP_D = Plant_Dynamic_Params.AC_COP_Factor;
    ACLoad_Power_D = Plant_Dynamic_Params.ACLoad_Power_Factor;
    T_AC_Base_D = Plant_Dynamic_Params.T_AC_SetPoint;
    T_AC_DeadBand_D = Plant_Dynamic_Params.T_AC_DeadBand;
    T_AC_HeatingMode_DeadBand_D = Plant_Dynamic_Params.T_AC_HeatingMode_DeadBand;
    AC_StartUp_LRA_Factor_D = Plant_Dynamic_Params.AC_StartUp_LRA_Factor;

    PV_RatedPower_D = Plant_Dynamic_Params.PV_RatedPower_Factor;

    Battery_Energy_Max_D = Plant_Dynamic_Params.Battery_Energy_Max_Factor;
    ...
    hem=hem_D;        % 1 - Eastern Hemisphere ; -1 - Western Hemisphere (-1)
    Lat=Lat_D;     % Decimal Deg (18.17)
    Long= Long_D;   % Decimal Deg (66.74)
    TimeZone=4;    % Hours
    Ltm=Ltm_D;        % Deg Decimal - Time Meridian (Atlantic Standard Time) (60)
    ...
    % Controller Load
    ACLoad_Num=1;
    ACLoad_Power=3000*ACLoad_Power_D; % Tesla Site Value
    
    AC_VolatageDip=0.3; % 30% Volage Dip at Startup causes 30% Current Dip
    
    %ac start up current
    AC_StartUp_LRA_Factor=AC_StartUp_LRA_Factor_D; % LRA - Locked Rotor Current usually 3-8 times rated current
    ACLoad_StartUp_Power=(1-AC_VolatageDip)^(2)*AC_StartUp_LRA_Factor*ACLoad_Power/1000; % In kW
    
    AC_COP=3.33*AC_COP_D;
    T_AC_Base=T_AC_Base_D;
    T_AC_DeadBand = T_AC_DeadBand_D;
    T_AC_HeatingMode_DeadBand = T_AC_HeatingMode_DeadBand_D;
    ...
    PV_RatedPower=325*PV_RatedPower_D;      % User Input Watts
    ...
    MaxRate_Charging=7.6*Battery_Energy_Max_D;
    ...
    MaxRate_Discharging=5*Battery_Energy_Max_D;
    MaxRate_Discharging_StartUp=7*Battery_Energy_Max_D;
    
    Battery_Energy_Max = 13.5*Battery_Energy_Max_D; % 13.5
    ...
    HEMSPlant_Params.T_AC_HeatingMode_DeadBand=T_AC_HeatingMode_DeadBand; 
