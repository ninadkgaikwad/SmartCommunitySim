function [U_k] = HEMS_Dumb_LocalController_OnGrid(X_k_Plant,W_k_Plant,HEMSPlant_Params,Community_Params,Simulation_Params)
...
T_AC_HeatingMode_DeadBand = HEMSPlant_Params.T_AC_HeatingMode_DeadBand;
...
for jj=1:N_House
    ...
    Baseline_ACController_Input.T_AC_HeatingMode_DeadBand=T_AC_HeatingMode_DeadBand;
    Baseline_ACController_Input.u_k_hvac_prev=X_k_Plant(1,30,jj);
    Baseline_ACController_Input.u_k_HeatingMode_prev=X_k_Plant(1,39,jj);
    Baseline_ACController_Input.T_house_now=X_k_Plant(1,7,jj);

    [u_k_ac, u_k_HeatingMode] = HEMS_AC_Controller_WithHeatingMode(Baseline_ACController_Input);

    U_k(1,3,jj)=u_k_ac; % Updating U_ac
    U_k(1,13,jj)=u_k_HeatingMode; % Updating U_HeatingMode
...
E_Load_Total_Desired=sum(E_Load_Desired)+(sum(U_k(1,3,:))*E_AC);
...
for jj=1:(N_PV_Bat+N_Bat)
    ...
    % Creating Input for Battery Logic Controller
    Baseline_BatteryController_Input.E_bat_CurrentHouse = E_bat_CurrentHouse;
    Baseline_BatteryController_Input.Battery_Energy_Max = Battery_Energy_Max;

    % Computing Control Command for Current House Battery
    [c_k,d_k] = HEMS_Battery_Controller_OnGrid(Baseline_BatteryController_Input);
...
for jj=[1:N_PV_Bat, N_PV_Bat+N_Bat+1:N_PV_Bat+N_Bat+N_PV]

    % Creating Input for PV Logic Controller
    Baseline_PVController_Input.PVEnergy_Available = PVEnergy_Available;

    % Computing Control Command for Current House PV
    [pv_con] = HEMS_PV_Controller_OnGrid(Baseline_PVController_Input);
