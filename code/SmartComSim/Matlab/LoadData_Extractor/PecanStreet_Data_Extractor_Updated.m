function [PecanStreet_Data_Output] = PecanStreet_Data_Extractor_Updated(HEMSWeatherData_Output,Simulation_Params,PecanStreet_Data_FolderPath,N_House_Vector,Type,Data_MatFile_Name)
...
%% Getting Required data from HEMSWeatherData_Output - Struct

%---------------------- HEMSWeatherData_Output ---------------------------%
 DateTime_Matrix=HEMSWeatherData_Output.DateTime_Matrix;

% Getting length HEMSWeatherData_Output.Ws
[File_Len, File_Width] = size(DateTime_Matrix);
...
for ii=1:length(AllFiles_Data_Cell) % For each file AllFiles_Data_Cell
    ...
    % Getting Current Load Data File
    Single_DataFile_Matrix=AllFiles_Data_Cell{1,ii};

    % Getting Size of Single_DataFile_Matrix
    [R_Current_DataFile, C_Current_Data_File] = size(Single_DataFile_Matrix);
    
    % Initializing Current Load Data Matrix
    Current_LoadData_Matrix = nan(File_Len, C_Current_Data_File);

    % Based on Length of Single_DataFile_Matrix we have
    if (R_Current_DataFile >= File_Len) % Load Data File has more data than required

        % Load Data in Current_LoadData_Matrix (one-shot)
        Current_LoadData_Matrix = Single_DataFile_Matrix(1:File_Len,:);

    elseif (R_Current_DataFile < File_Len)

        % Getting indices for NaN rows of Current_LoadData_Matrix
        NaN_Rows_Current_LoadData_Matrix = find(all(isnan(Current_LoadData_Matrix), 2));

        % Getting Length of NaN_Rows_Current_LoadData_Matrix
        Len_NaN_Rows_Current_LoadData_Matrix = length(NaN_Rows_Current_LoadData_Matrix);

        while (Len_NaN_Rows_Current_LoadData_Matrix > 0)

            if (R_Current_DataFile <= Len_NaN_Rows_Current_LoadData_Matrix )

                % Load Data in Current_LoadData_Matrix (one-shot)
                Current_LoadData_Matrix(NaN_Rows_Current_LoadData_Matrix(1):NaN_Rows_Current_LoadData_Matrix(1)+R_Current_DataFile-1,:) = Single_DataFile_Matrix(1:R_Current_DataFile,:);

            elseif (R_Current_DataFile > Len_NaN_Rows_Current_LoadData_Matrix)

                % Load Data in Current_LoadData_Matrix (one-shot)
                Current_LoadData_Matrix(NaN_Rows_Current_LoadData_Matrix(1):end,:) = Single_DataFile_Matrix(1:Len_NaN_Rows_Current_LoadData_Matrix,:);
    ...
    AllFiles_CorrectDates_Data_Matrix(:,:,ii)=Current_LoadData_Matrix;  
