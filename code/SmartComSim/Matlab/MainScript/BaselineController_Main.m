...
% Date: Feb/1/2021
% Description: Main File for Baseline Controller for Large House
...
OS=2; % 1 - Linux ; 2 - Windows-Laptop ; 3 - Windows-PC; 4-shishir's window PC
...
N1=0;                           % User Input - Battery Max Changing Factor
...
ImageFolder_Name='Gainesville_KPEC_OffGrid_Smart_';

SimulationData_FileName='FigurePlotterData_Gainesville_KPEC_OffGrid_Smart_WOAC';

SimulationPerformanceData_FileName='PerformanceData_Gainesville_KPEC_OffGrid_Smart_WOAC';

LoadData_FileName='PecanStreet_LoadData_KPEC2025';

WeatherData_FileName='Gainesville_Irma_KPEC2025';
...
elseif (OS==2) % Windows-Laptop
     ...
     WeatherDataFile_Path='C:\Users\ninad\OneDrive - Washington State University (email.wsu.edu)\20_Gaikwad_SmartCommunity\data\WeatherData\Gainesville_2017_To_2017_WeatherData_NSRDB_30minTo10minRes.csv';
    
     LoadDataFolder_Path='C:\Users\ninad\OneDrive - Washington State University (email.wsu.edu)\20_Gaikwad_SmartCommunity\data\PreProcessedFiles\10minute_data_austin_HouseWise\';
...
Simulation_Params.OffGrid_Simulation_ModeType = OffGrid_Simulation_ModeType;
...
% Add Control of PV Current as column  - Not Needed
% Add Control of Heating_Mode Current as column - Not Needed
% Add Energy from Grid as column 39
if (Simulation_ModeType == 0) % Off-Grid

    %X_k_Plant=zeros(2,38,N_House); % Initializing Size

    X_k_Plant=zeros(2,39,N_House); % Initializing Size
...
for ii=1:N_House
    ...
    % AC Heating_Mode Status Previous
    X_k_Plant(1,39,ii)=[0];
    
    % Prioritized Loads on-off Status Previous
    X_k_Plant(1,31:38,ii)=ones(1,8,1);    
...
% Add control of PV as last column (12)
% Add control of AC Heating Mode as last column (13)
U_k_History=zeros(1,13,N_House);

if (Simulation_ModeType == 0) % Off-Grid

    % U_k_History=zeros(1,11,N_House); % Initializing Size

    U_k_History=zeros(1,13,N_House); % Initializing Size
...
for ii=1:Simulation_Steps_Total % For each Simulation Time Step
    ...
    if (Simulation_ModeType == 0) % Off-Grid Mode
        ...
        [X_k_Plus_Plant] = HEMS_Plant_OffGrid(X_k_Plant,W_k_Plant,U_k,HEMSPlant_Params,HEMSHouse_Params,Community_Params,Simulation_Params);
    
    elseif (Simulation_ModeType == 1) % On-Grid Mode

        [X_k_Plus_Plant] = HEMS_Plant_OnGrid(X_k_Plant,W_k_Plant,U_k,HEMSPlant_Params,HEMSHouse_Params,Community_Params,Simulation_Params);
