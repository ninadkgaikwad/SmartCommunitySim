function [X_k_Plus_Plant] = HEMS_Plant_OffGrid(X_k_Plant,W_k_Plant,U_k,HEMSPlant_Params,HEMSHouse_Params,Community_Params,Simulation_Params)
...
OffGrid_Simulation_ModeType = Simulation_Params.OffGrid_Simulation_ModeType;
...
if (OffGrid_Simulation_ModeType == 1) % With AC Start-up Constraint
    ...
    if (E_Mis>=0) % All Loads can be serviced
        ...
        if(Total_StartUpPower_Available>=Total_StartUpPower_AC) % Enough Batteries/PV to Power the Turn On of ACs
            ...
            for jj=1:N_House % For each House in Smart Comuunity       
                ...
                HEMSHouse_States.u_k_HeatingMode=U_k(1,13,jj); % New
...
elseif (OffGrid_Simulation_ModeType == 0) % Without AC Start-up constraint
